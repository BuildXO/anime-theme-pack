...
import {
  ...
  BACKGROUND_COMMENT,
  CONFIG_NAME,
  CONFIG_BACKGROUND_ENABLED,
  CONFIG_BACKGROUND_PATH,
  CONFIG_BACKGROUND_OPACITY,
  CONFIG_BACKGROUND_ANCHOR,
  CONFIG_BACKGROUND_BLUR,
  InstallStatus,
  ThemeDefinition
} from './constants';
import { loadImageBase64FromFile, fileExists } from './utils';
...
export function canWrite(): boolean {
  try {
    ...
    if (!fs.existsSync(editorCss)) {
      console.error('[Anime Theme] CSS file does not exist:', editorCss);
      return false;
    }
 ...
 */
function writeCSS(css: string): void {
  try {
    console.log('[Anime Theme] Writing CSS to:', editorCss);
    console.log('[Anime Theme] CSS length:', css.length, 'bytes');
    fs.writeFileSync(editorCss, css, 'utf-8');
 ...
 */
function getBackgroundIndex(css: string): number {
  return css.indexOf(BACKGROUND_COMMENT);
}

/**
 * Remove background CSS from the provided CSS string
 */
function removeBackgroundCSS(css: string): string {
...
function buildBackgroundCSS(
  ...
  const isEnabled = config.get(CONFIG_BACKGROUND_ENABLED, true);
...
export function installBackground(
  ...
  if (!canWrite()) {
    console.error('[Anime Theme] Cannot write to VSCode CSS file:', editorCss);
    return InstallStatus.FAILURE;
  }

  try {
    const config = vscode.workspace.getConfiguration(CONFIG_NAME);
    const customBackgroundPath = config.get<string>(CONFIG_BACKGROUND_PATH);
    const opacity = config.get<number>(CONFIG_BACKGROUND_OPACITY, 0.15);
    const anchor = config.get<string>(CONFIG_BACKGROUND_ANCHOR, 'center');
    const blur = config.get<number>(CONFIG_BACKGROUND_BLUR, 0);

    // Determine which background image to use
    let backgroundImagePath: string;
    
    if (customBackgroundPath && fileExists(customBackgroundPath)) {
    ...
    if (!fileExists(backgroundImagePath)) {
      ...
      vscode.window.showErrorMessage(
        `Background image not found for ${theme.label}. Please ensure the image exists at: images/background_images/${theme.backgroundImage}`
      );
      return InstallStatus.FAILURE;
    }
    ...
    const backgroundDataURL = loadImageBase64FromFile(backgroundImagePath);
 ...
 */
export function removeBackground(): InstallStatus {
  if (!canWrite()) {
...
const TRANSPARENT_COLORS: Record<string, string> = {
...
const BALANCING_COLORS: Record<string, string> = {
 ...
 */
export async function applyTransparentColors(): Promise<void> {
  const themeName = vscode.workspace.getConfiguration('workbench').get<string>('colorTheme') ?? '';
  const config = vscode.workspace.getConfiguration();
  const existing = config.get<Record<string, unknown>>('workbench.colorCustomizations', {});
  const key = `[${themeName}]`;
  ...
  const updated = {
    ...existing,
    [key]: {
      ...(existing[key] as Record<string, string> | undefined ?? {}),
      ...colorsToApply,
    },
  };
 ...
 */
export async function removeTransparentColors(): Promise<void> {
  const themeName = vscode.workspace.getConfiguration('workbench').get<string>('colorTheme') ?? '';
  const config = vscode.workspace.getConfiguration();
  ...
  if (existing[key]) {
    const themeSection = { ...(existing[key] as Record<string, string>) };
 ...
 */
export function isBackgroundInstalled(): boolean {
  try {
    if (!fs.existsSync(editorCss)) {
      return false;
    }
